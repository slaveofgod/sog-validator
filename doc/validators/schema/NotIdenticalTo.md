# NotIdenticalTo
Validates that a value is **not** identical to another value, defined in the options. To force that a value is identical, see [IdenticalTo][identicalto-schema-url].

This constraint compares using `!==,` so `3` and "`3`" are considered not equal. Use [NotEqualTo][notequalto-schema-url] to compare with `!=`.
