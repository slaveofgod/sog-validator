...
class CardSchemeValidator extends Validator {
    constructor() {
        ...
        var schemes = arguments[0]['schemes'];

        this.configuring();

        this.setMessage('message', message);

        if(this.isEmpty(schemes)){
            throw new Error('Either option "schemes" must be given');
        }else if(
    ...
    validate(data, errorType) {
        ...
        var schemes = this.getParameter('schemes');
        for (var schemeKey in schemes){
            var rules = this.schemes[schemes[schemeKey]];
